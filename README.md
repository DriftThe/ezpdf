...
- **翻译范围可控**：在 设置 → 常规 中勾选参与翻译的块类型（正文、标题、脚注、页脚等）；未勾选的类型不翻译也不覆盖，原 PDF 像素直出。改动只对尚未翻译的页面生效。
...
3. **配置翻译模型**：进入 设置 → **LLM**，依次选择「供应商」与「预设模型」（亦可手动填写模型名），填入 **API Key**，并点击「验证」确认连通性。默认使用 OpenAI 兼容协议。若暂时不需要翻译，可关闭该页首项的「**启用翻译**」：OCR 出的文本会直接以原文记入译文列并标记完成，之后重新打开翻译也不会回翻这些页面。
4. **准备 OCR 服务**：进入 设置 → **OCR** 选择「服务来源」。
   ...
   - **在线服务**：填入一个已部署的同款解析服务地址（如 `http://127.0.0.1:9055`），点右侧「测试」探活——无需本地依赖与模型，批大小由该服务公布（每次请求前自动重新协商）。协议与自建服务所需的完整契约见 [`pyserver/PROTOCOL.md`](pyserver/PROTOCOL.md)，开发联调可执行 `python pyserver/server_test.py`（默认 9055）。
...
- **其他协议**：当前仅支持 OpenAI 兼容协议（`openai-completions`）的端点，因此供应商下拉中不再列出其他协议的供应商（旧配置指向它们时显示为禁用项）。原因与接入方式见 [`docs/protocols.md`](docs/protocols.md)。
